...
# 
import os
from kapp_func_ys_multiline_typedecls_test import KAppFuncYSMLTTest

class CustomTest(KAppFuncYSMLTTest):

    def config(self, myname, result):

        result[myname]['prerun_build'] = 'module swap intel gnu/6.1.0'
        result[myname]['prerun_run'] = 'module swap intel gnu/6.1.0'
    ...
    def generate(self, myname, result):
        ...
        passed, out, err = self.extract_kernel(os.path.join(tmpsrc, 'calling_module.F90'), \
            None, \
            __invocation='0:0:0', \
            __cmd_clean='"cd %s; make clean FC=gfortran"'%tmpsrc, \
            __cmd_build='"cd %s; make build FC=gfortran"'%tmpsrc, \
            __cmd_run='"cd %s; make run FC=gfortran"'%tmpsrc, \
            __rebuild='all', \
