...
sharegendir = os.path.join(here, "..", "..", "genfiles", "csm_share")
...
class Test(KAppSysCHCesmIntelTest):
    ...
    def generate(self, myname, result):
        ...
        dst = os.path.join(result['config_task']['cesmtmpdir'], "bld", "intel", "mpt",
            "nodebug","nothreads", "mct", "noesmf", "c1a1l1i1o1r1g1w1e1", "csm_share")
        if not os.path.exists(dst):
            os.makedirs(dst)

        for f in os.listdir(sharegendir):
            src = os.path.join(sharegendir, f)
            if os.path.isfile(src):
                shutil.copyfile(src, os.path.join(dst, f))

        # copy atm src files
        dst = os.path.join(result['config_task']['cesmtmpdir'], "bld", "atm", "obj")
        if not os.path.exists(dst):
            os.makedirs(dst)
