...
class KAppSysYSCesmPgiTest(KAppSysYSCesmTest):
    def config(self, myname, result):
        ...
        if self.REBUILD or not os.path.exists(datadir) or len([name for name in os.listdir(datadir) if os.path.isfile(os.path.join(datadir, name))])==0 or \
            ...
            if not os.path.exists(casedir):
                casecmd = './create_newcase -project %s -mach yellowstone -compset FC5 -res ne16_ne16 -compiler pgi -case %s'%(self.OPTIONS['project'], casedir)
