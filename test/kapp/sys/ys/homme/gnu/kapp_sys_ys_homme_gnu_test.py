...
class KAppSysYSHommeGnuTest(KAppSysYSHommeTest):
    ...
    def config(self, myname, result):
        ...
        if self.REBUILD or not os.path.exists(blddir) or len([name for name in os.listdir(blddir) if os.path.isfile(os.path.join(blddir, name))])==0:
            ...
            cmake_cmd.append('-DCMAKE_C_COMPILER="mpcc"')
            cmake_cmd.append('-DCMAKE_CXX_COMPILER="mpCC"')
            cmake_cmd.append('-DCMAKE_Fortran_COMPILER="mpfort"')
