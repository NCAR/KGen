...
class Test(KAppSysYSCalcCMSTest):
    def generate(self, myname, result):
        ...
        prerun = 'module swap intel pgi/15.10'

        passed, out, err = self.extract_kernel(srcfile, None, \
            __cmd_clean='"cd %s; make -f Makefile.lsf clean"'%tmpsrc, \
            __cmd_build='"cd %s; make -f Makefile.lsf build"'%tmpsrc, \
            __cmd_run='"cd %s; make -f Makefile.lsf run"'%tmpsrc, \
