...
#from .dependency import CHECKERS, DbmDB, JsonDB, SqliteDB, Dependency # KGEN deletion
from .dependency import CHECKERS, JsonDB, SqliteDB, Dependency # KGEN addition
...
opt_backend = {
    ...
    'long': 'backend',
    'type': str,
    #'default': "dbm", # KGEN deletion
    'default': "sqlite3", # KGEN addition
...
class DoitCmdBase(Command):
    ...
    def get_backends(self):
        ...
        #backend_map = {'dbm': DbmDB, 'json': JsonDB, 'sqlite3': SqliteDB} # KGEN deletion
        backend_map = {'json': JsonDB, 'sqlite3': SqliteDB} # KGEN  addition
        # add plugins
        plugins = PluginDict()
    ...
    def execute(self, params, args):
        ...
        #args_name = list(inspect.signature(self._execute).parameters.keys()) # KGEN deletion
        args_name = list(signature(self._execute).parameters.keys()) # KGEN deletion
        # start of KGEN addition
#        args_name = []
#        (args, varargs, keywords, defaults) = inspect.getargspec(self._execute)
#        if args:
#            for arg in args:
#                if isinstance(arg, str) and arg and arg!='self':
#                    args_name.append(arg) 
#        if varargs:
#            for arg in varargs:
#                if isinstance(arg, str) and arg and arg!='self':
#                    args_name.append(arg) 
