...
! KGEN version: 0.4.12
    ...
    MODULE test7_mod
        ...
        SUBROUTINE test_7(kgen_unit)
            ...
            LOGICAL :: a
            LOGICAL :: b
            REAL(KIND=4), dimension(2,2) :: ar3
            REAL(KIND=4) :: ref_ar3(2,2)
            REAL(KIND=4), dimension(2,2) :: ar1
            REAL(KIND=4), dimension(2,2) :: ar2
            tolerance = 1.E-14
            CALL kgen_init_check(check_status, tolerance)
            READ(UNIT=kgen_unit) a
            READ(UNIT=kgen_unit) b
            READ(UNIT=kgen_unit) ar3
            READ(UNIT=kgen_unit) ar1
            READ(UNIT=kgen_unit) ar2

            READ(UNIT=kgen_unit) ref_ar3


            ! call to kernel
                call add(a, b, ar1, ar2, ar3)
            ! kernel verification for output variables
            CALL kgen_verify_real_4_dim2( "ar3", check_status, ar3, ref_ar3)
        ...
        ! write subroutines
            SUBROUTINE kgen_read_real_4_dim2(var, kgen_unit, printvar)
                INTEGER, INTENT(IN) :: kgen_unit
                ...
                IF ( is_true ) THEN
                    ...
                    END IF
                END IF
            END SUBROUTINE kgen_read_real_4_dim2


        ! verify subroutines
            SUBROUTINE kgen_verify_real_4_dim2( varname, check_status, var, ref_var)
                ...
                END IF
            END SUBROUTINE kgen_verify_real_4_dim2
