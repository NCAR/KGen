...
#SRC_DIR=/glade/p/work/amogh/kgen_dev/regression_tests/unit_tests/test_6/src
#SRC=${SRC_DIR}/test3_mod.F90
#BUILD_DIR=${SRC_DIR}
#CALLSITE=test3_mod.test_3.add

python ${KGEN} \
	-i include.ini \
	-e exclude.ini \
	--state-build cmds="cd ${BUILD_DIR}; make build" \
	--state-run cmds="cd ${BUILD_DIR}; ./test6.exe" \
	${SRC}:${CALLSITE}
	#-D HAVE_CONFIG_H=1 \
