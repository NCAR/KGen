...
def search_Attr_Spec(stmt, node, gentype=None): 
    ...
    defer_items(stmt, node, gentype=KGGenType.STATE_IN)
...
def search_Int_Literal_Constant(stmt, node, gentype=None): 
    ...
    if node.items[1]:
        get_name_or_defer(stmt, Fortran2003.Name(node.items[1]), res_typedecl, gentype=None)
...
def search_Comment(stmt, node, gentype=None): 
    ...
    if hasattr(stmt, 'write_state'):
        for var in stmt.write_state:
            ...
            get_name_or_defer(stmt, f2003obj, res_typedecl, gentype=KGGenType.STATE_IN)
...
def search_Loop_Control(stmt, node, gentype=None): 
    ...
    if len(node.items)==1:
        ...
        get_name_or_defer(stmt, node.items[0], res_typedecl, gentype=KGGenType.STATE_INOUT)
        if isinstance(node.items[1], list):
            for item in node.items[1]:
                get_name_or_defer(stmt, item, res_value, gentype=KGGenType.STATE_IN)
        else:
            get_name_or_defer(stmt, node.items[1], res_value, gentype=KGGenType.STATE_IN)
...
def search_Function_Stmt(stmt, node, gentype=None): 
    ...
    get_name_or_defer(stmt, node.items[0], res_derivedtype, gentype=KGGenType.STATE_IN ) # prefix
    get_name_or_defer(stmt, node.items[2], res_typedecl, gentype=None) # dummy args
    get_name_or_defer(stmt, node.items[3], res_typedecl, gentype=KGGenType.STATE_IN)
...
def search_Allocate_Stmt(stmt, node, gentype=None): 
    ...
    get_name_or_defer(stmt, node.items[0], res_typespec, gentype=KGGenType.STATE_IN)
    get_name_or_defer(stmt, node.items[1], res_typedecl, gentype=gentype)
    defer(stmt, node.items[2], gentype=KGGenType.STATE_IN)

def search_Allocation(stmt, node, gentype=None): 
    """ Identifying a name in Allocation node"""
    get_name_or_defer(stmt, node.items[0], res_typedecl, gentype=gentype)
    get_name_or_defer(stmt, node.items[1], res_value, gentype=KGGenType.STATE_IN)
    #if len(node.items)>1:
...
def search_Label_Do_Stmt(stmt, node, gentype=None): 
    ...
    defer(stmt, node.items[2], gentype=None)
...
def search_Call_Stmt(stmt, node, gentype=None): 
    ...
    get_name_or_defer(stmt, node.items[0], res_subroutine, gentype=None)
    ...
    get_name_or_defer(stmt, node.items[1], res_value, gentype=None)
...
def search_Type_Param_Value(stmt, node, gentype=None): 
    ...
    if node.item:
        get_name_or_defer(stmt, node.item, res_value, gentype=KGGenType.STATE_IN)
...
def search_Procedure_Stmt(stmt, node, gentype=None):
    ...
    get_name_or_defer(stmt, node.items[0], res_subprogram, gentype=None)

def search_Prefix(stmt, node, gentype=None):
    """ Identifying a name in Prefix node"""
    for item in node.items:
        get_name_or_defer(stmt, node.items[0], res_anything, gentype=KGGenType.STATE_IN)
...
def search_Declaration_Type_Spec(stmt, node, gentype=None):
    ...
    get_name_or_defer(stmt, node.items[1], res_derivedtype, gentype=KGGenType.STATE_IN)
...
def search_Data_Ref(stmt, node, gentype=None):
    ...
    if isinstance(node.items[0], Name):
        ...
        get_name_or_defer(stmt, node.items[0].items[1], res_value, gentype=KGGenType.STATE_IN) 

    for item in node.items[1:]:
        if isinstance(item, Name): pass
        elif isinstance(item, Part_Ref):
            get_name_or_defer(stmt, item.items[1], res_value, gentype=KGGenType.STATE_IN)
...
def search_Structure_Constructor(stmt, node, gentype=None):
    ...
    get_name_or_defer(stmt, node.items[0], res_value + res_derivedtype, gentype=KGGenType.STATE_IN)
    get_name_or_defer(stmt, node.items[1], res_value, gentype=KGGenType.STATE_IN)
...
def search_Access_Stmt(stmt, node, gentype=None):
    ...
    get_name_or_defer(stmt, node.items[1], res_anything, gentype=None)

def search_Function_Reference(stmt, node, gentype=None):
    """ Identifying a name in Function_Reference node"""
    get_name_or_defer(stmt, node.items[0], res_function, gentype=None)
    get_name_or_defer(stmt, node.items[1], res_value, gentype=None)

def search_Return_Stmt(stmt, node, gentype=None):
    """ Identifying a name in Return_Stmt node"""
    get_name_or_defer(stmt, node.items[0], res_function, gentype=KGGenType.STATE_OUT)
...
def search_Pointer_Assignment_Stmt(stmt, node, gentype=None):
    ...
    get_name_or_defer(stmt, node.items[0], res_ptr_object, gentype=gentype) # data pointer obj or procedure pointer obj
    get_name_or_defer(stmt, node.items[2], res_target, gentype=gentype) # data target or procedure target
...
def search_Deallocate_Stmt(stmt, node, gentype=None):
    ...
    get_name_or_defer(stmt, node.items[0], res_value, gentype=None)
    get_name_or_defer(stmt, node.items[1], res_value, gentype=None)
...
def search_External_Stmt(stmt, node, gentype=None):
    ...
    get_name_or_defer(stmt, node.items[1], \
        [ TypeDeclarationStatement, Function, Subroutine ], gentype=None )
...
def search_Wait_Spec(stmt, node, gentype=None):
    ...
    if hasattr(node, 'items') and len(node.items)>0:
        for item in node.items:
            get_name_or_defer(stmt, item, res_value, gentype=None)
...
def search_Data_Stmt(stmt, node, gentype=None):
    ...
    if hasattr(node, 'items') and len(node.items)>0:
        for item in node.items:
            get_name_or_defer(stmt, item, res_typedecl, gentype=None)

def search_Data_Stmt_Value(stmt, node, gentype=None):
    """ Identifying a name in Data_Stmt_Value node"""
    get_name_or_defer(stmt, node.items[0], res_typedecl, gentype=KGGenType.STATE_IN)
    get_name_or_defer(stmt, node.items[1], res_typedecl, gentype=KGGenType.STATE_IN)

def search_Save_Stmt(stmt, node, gentype=None):
    """ Identifying a name in Save_Stmt node"""
    get_name_or_defer(stmt, node.items[1], res_typedecl, gentype=None)
    ...
    if hasattr(node, 'items') and len(node.items)>0:
        for itemlist in node.items:
            for name, _item in itemlist:
                get_name_or_defer(stmt, _item, res_value, gentype=None)
...
def search_Equivalence_Set(stmt, node, gentype=None):
    ...
    get_name_or_defer(stmt, node.items[0], res_typedecl, gentype=None)
    get_name_or_defer(stmt, node.items[1], res_typedecl, gentype=None)
...
def search_Namelist_Stmt(stmt, node, gentype=None):
    ...
    if hasattr(node, 'items') and len(node.items)>0:
        for nlname, nlgroup in node.items:
            get_name_or_defer(stmt, nlgroup, res_typedecl, gentype=None)
...
def search_Target_Stmt(stmt, node, gentype=None):
    ...
    get_name_or_defer(stmt, node.items[0], res_anything, gentype=None)

def search_Target_Entity_Decl(stmt, node, gentype=None):
    """ Identifying a name in Target_Entity_Decl node"""
    get_name_or_defer(stmt, node.items[0], res_anything, gentype=gentype)
    defer(stmt, node.items[1], gentype=KGGenType.STATE_OUT)
    get_name_or_defer(stmt, node.items[2], res_value, gentype=KGGenType.STATE_IN)
    get_name_or_defer(stmt, node.items[3], res_value, gentype=KGGenType.STATE_IN) 

def search_Volatile_Stmt(stmt, node, gentype=None):
    """ Identifying a name in Volatile_Stmt node"""
    get_name_or_defer(stmt, node.items[1], res_anything, gentype=KGGenType.STATE_IN)
...
def search_Inquire_Stmt(stmt, node, gentype=None):
    ...
    get_name_or_defer(stmt, node.items[0], res_value, gentype=None)
    get_name_or_defer(stmt, node.items[1], res_value, gentype=None)
    get_name_or_defer(stmt, node.items[2], res_value, gentype=None)
...
def search_Saved_Entity(stmt, node, gentype=None):
    ...
    if len(node.items)==3 and node.items[0]=='/' and node.items[2]=='/':
        get_name_or_defer(stmt, node.items[1], res_common, gentype=None)
    else:
        for item in node.items:
            get_name_or_defer(stmt, item, res_value, gentype=None)
...
def search_Specific_Binding(stmt, node, gentype=None):
    ...
    get_name_or_defer(stmt, node.items[0], res_typespec + [ Interface ], gentype=None)
    get_name_or_defer(stmt, node.items[1], res_value, gentype=None)
    if node.items[3] is None:
        get_name_or_defer(stmt, node.items[2], res_subprogram, gentype=None)
    else:
        get_name_or_defer(stmt, node.items[3], res_subprogram, gentype=None)
...
def search_Generic_Binding(stmt, node, gentype=None):
    ...
    get_name_or_defer(stmt, node.items[0], res_value, gentype=KGGenType.STATE_IN)
    get_name_or_defer(stmt, node.items[2], [ Fortran2003.Specific_Binding ], gentype=KGGenType.STATE_IN)
...
def search_Data_Implied_Do(stmt, node, gentype=None):
    ...
    get_name_or_defer(stmt, node.items[2], res_value, gentype=KGGenType.STATE_IN)
    get_name_or_defer(stmt, node.items[3], res_value, gentype=KGGenType.STATE_IN)
    get_name_or_defer(stmt, node.items[4], res_value, gentype=KGGenType.STATE_IN)

def search_Ac_Spec(stmt, node, gentype=None):
    """ Identifying a name in Ac_Spec node"""
    defer(stmt, node.items[0], gentype=KGGenType.STATE_IN)
    get_name_or_defer(stmt, node.items[1], res_value, gentype=None)
