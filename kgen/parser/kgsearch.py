...
def f2003_search_unknowns(cfg, stmt, node, resolvers=None, gentype=None):
    ...
    if clsname=='Name':
        get_name(cfg, stmt, node, resolvers, gentype=gentype)
        return
    ...
    try:
        if clsname.endswith('_List'):
            ...
            for item in node.items:
                ...
                if itemclsname=='Name':
                    get_name(cfg, stmt, item, resolvers, gentype=gentype)
                else:
                    exec('search_%s(cfg, stmt, item, gentype=gentype)' % itemclsname)
        elif clsname.startswith('End_'):
            pass
        else:
            exec('search_%s(cfg, stmt, node, gentype=gentype)' % clsname)
        ...
        if cfg.search['promote_exception']:
            raise
        else:
            print ''
            print errmsg
...
def get_name_or_defer(cfg, stmt, node, resolvers, defer=True, gentype=None):
    ...
    if isinstance(node, Fortran2003.Name):
        ...
        if node.string.lower() in Intrinsic_Procedures:
            if  cfg.search['skip_intrinsic'] and not is_except(cfg, node, stmt):
            ...
            elif not cfg.search['skip_intrinsic'] and is_except(cfg, node, stmt): 
                if hasattr(node, 'parent') and not isinstance(node.parent, Fortran2003.Part_Ref) and \
                    not (isinstance(node.parent, Fortran2003.Function_Reference) and node.string.lower()=='null') and \
                    not (isinstance(node.parent, Fortran2003.Specific_Binding) and node.string.lower()=='null'):
        ...
        # skip if excluded
        #if cfg.exclude.has_key('namepath') and stmt.__class__ in execution_part:
        if cfg.exclude.has_key('namepath'):
            for pattern, actions in cfg.exclude['namepath'].iteritems():
                ...
                if match_namepath(pattern, namepath):
                    ...
                    if not hasattr(cfg, stmt, 'exclude_names'): stmt.exclude_names = OrderedDict()
        ...
        f2003_search_unknowns(cfg, stmt, node, resolvers, gentype=gentype)

def get_name(cfg, stmt, node, resolvers, gentype=None):
    get_name_or_defer(cfg, stmt, node, resolvers, defer=False, gentype=gentype)

def defer(cfg, stmt, node, gentype=None):
    if isinstance(node, Fortran2003.Name):
        raise SearchException('%s can not be Name class' % str(node))
    f2003_search_unknowns(cfg, stmt, node, gentype=gentype)


def defer_items(cfg, stmt, node, gentype=None):
    if hasattr(node, 'items'):
        for item in node.items:
            if isinstance(item, Fortran2003.Name):
                raise SearchException('%s can not be Name class' % str(item))
            f2003_search_unknowns(cfg, stmt, item, gentype=gentype)

###############################################################################
################################### SEARCH ####################################
###############################################################################

def search_Type_Declaration_Stmt(cfg, stmt, node, gentype=None):  
    ...
    defer_items(cfg, stmt, node)

def search_Intrinsic_Type_Spec(cfg, stmt, node, gentype=None): 
    """ Identifying a name in Intrinsic_Type_Spec node"""
    defer(cfg, stmt, node.items[1])

def search_Kind_Selector(cfg, stmt, node, gentype=None): 
    """ Identifying a name in Kind_Selector node"""
    get_name_or_defer(cfg, stmt, node.items[1], res_kind)

def search_Entity_Decl(cfg, stmt, node, gentype=None): 
    """ Identifying a name in Entity_Decl node"""
    defer(cfg, stmt, node.items[1])
    get_name_or_defer(cfg, stmt, node.items[2], res_value)
    get_name_or_defer(cfg, stmt, node.items[3], res_value) 

def search_Explicit_Shape_Spec(cfg, stmt, node, gentype=None): 
    """ Identifying a name in Explicit_Shape_Spec node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_value)
    get_name_or_defer(cfg, stmt, node.items[1], res_value)

def search_Dimension_Attr_Spec(cfg, stmt, node, gentype=None): 
    """ Identifying a name in Dimension_Attr_Spec node"""
    defer(cfg, stmt, node.items[1])

def search_Add_Operand(cfg, stmt, node, gentype=None): 
    """ Identifying a name in Add_Operand node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_value)
    get_name_or_defer(cfg, stmt, node.items[2], res_value)

def search_Mult_Operand(cfg, stmt, node, gentype=None): 
    """ Identifying a name in Mult_Operand node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_value)
    get_name_or_defer(cfg, stmt, node.items[2], res_value)

def search_Attr_Spec(cfg, stmt, node, gentype=None): 
    """ Identifying a name in Attr_Spec node"""
    defer_items(cfg, stmt, node)

def search_Initialization(cfg, stmt, node, gentype=None): 
    """ Identifying a name in Initialization node"""
    get_name_or_defer(cfg, stmt, node.items[1], res_value)

def search_Part_Ref(cfg, stmt, node, gentype=None): 
    """ Identifying a name in Part_Ref node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_value, gentype=gentype) 
    get_name_or_defer(cfg, stmt, node.items[1], res_value) 

def search_Structure_Constructor_2(cfg, stmt, node, gentype=None): 
    """ Identifying a name in Structure_Constructor_2 node"""
    get_name_or_defer(cfg, stmt, node.items[1], res_value)

def search_Int_Literal_Constant(cfg, stmt, node, gentype=None): 
    """ Identifying a name in Int_Literal_Constant node"""
    if node.items[1]:
        get_name_or_defer(cfg, stmt, Fortran2003.Name(node.items[1]), res_typedecl)

def search_Signed_Int_Literal_Constant(cfg, stmt, node, gentype=None): 
    """ Identifying a name in Signed_Int_Literal_Constant node"""
    if node.items[1]:
        get_name_or_defer(cfg, stmt, Fortran2003.Name(node.items[1]), res_typedecl)

def search_Real_Literal_Constant(cfg, stmt, node, gentype=None): 
    """ Identifying a name in Real_Literal_Constant node"""
    if node.items[1]:
        get_name_or_defer(cfg, stmt, Fortran2003.Name(node.items[1]), res_typedecl)

def search_Signed_Real_Literal_Constant(cfg, stmt, node, gentype=None): 
    """ Identifying a name in Signed_Real_Literal_Constant node"""
    if node.items[1]:
        get_name_or_defer(cfg, stmt, Fortran2003.Name(node.items[1]), res_typedecl)

def search_Subroutine_Stmt(cfg, stmt, node, gentype=None): 
    """ Identifying a name in Subroutine_Stmt node"""
    get_name_or_defer(cfg, stmt, node.items[2], res_typedecl) # dummy args
    get_name_or_defer(cfg, stmt, node.items[3], res_typedecl) # postfix

def search_Comment(cfg, stmt, node, gentype=None): 
    ...
    if hasattr(stmt, 'write_state'):
        for var in stmt.write_state:
            ...
            get_name_or_defer(cfg, stmt, f2003obj, res_typedecl)

def search_Nonlabel_Do_Stmt(cfg, stmt, node, gentype=None): 
    """ Identifying a name in Nonlabel_Do_Stmt node"""
    if len(node.items)==3:
        defer(cfg, stmt, node.items[2])
    elif len(node.items)==2:
        if isinstance(node.items[0], str):
            defer(cfg, stmt, node.items[1])

def search_Loop_Control(cfg, stmt, node, gentype=None): 
    """ Identifying a name in Loop_Control node"""
    if len(node.items)==1:
        get_name_or_defer(cfg, stmt, node.items[0], res_value)
    else:
        get_name_or_defer(cfg, stmt, node.items[0], res_typedecl, gentype=KGGenType.STATE_OUT)
        if isinstance(node.items[1], list):
            for item in node.items[1]:
                get_name_or_defer(cfg, stmt, item, res_value)
        else:
            get_name_or_defer(cfg, stmt, node.items[1], res_value)

def search_Assignment_Stmt(cfg, stmt, node, gentype=None): 
    """ Identifying a name in Assignment_Stmt node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_value, gentype=KGGenType.STATE_OUT)
    get_name_or_defer(cfg, stmt, node.items[2], res_value)

def search_Level_2_Expr(cfg, stmt, node, gentype=None): 
    """ Identifying a name in Level_2_Expr node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_value)
    get_name_or_defer(cfg, stmt, node.items[2], res_value)

def search_Parenthesis(cfg, stmt, node, gentype=None): 
    """ Identifying a name in Parenthesis node"""
    get_name_or_defer(cfg, stmt, node.items[1], res_value, gentype=gentype)

def search_str(cfg, stmt, string, gentype=None):
    pass

def search_Function_Stmt(cfg, stmt, node, gentype=None): 
    """ Identifying a name in Function_Stmt node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_derivedtype ) # prefix
    get_name_or_defer(cfg, stmt, node.items[2], res_typedecl) # dummy args
    get_name_or_defer(cfg, stmt, node.items[3], res_typedecl)

def search_Assumed_Shape_Spec(cfg, stmt, node, gentype=None): 
    """ Identifying a name in Assumed_Shape_Spec node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_value)
    get_name_or_defer(cfg, stmt, node.items[1], res_value)

def search_Allocate_Stmt(cfg, stmt, node, gentype=None): 
    """ Identifying a name in Allocate_Stmt node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_typespec)
    get_name_or_defer(cfg, stmt, node.items[1], res_typedecl)
    defer(cfg, stmt, node.items[2])

def search_Allocation(cfg, stmt, node, gentype=None): 
    """ Identifying a name in Allocation node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_typedecl)
    get_name_or_defer(cfg, stmt, node.items[1], res_value)
    #if len(node.items)>1:
    #    defer_items(cfg, stmt, node.items[1:])

def search_Allocate_Shape_Spec(cfg, stmt, node, gentype=None): 
    """ Identifying a name in Allocate_Shape_Spec node"""
    if node.items:
        for item in node.items:
            get_name_or_defer(cfg, stmt, item, res_value)

def search_Use_Stmt(cfg, stmt, node, gentype=None): 
    """ Identifying a name in Use_Stmt node"""
    pass

def search_If_Then_Stmt(cfg, stmt, node, gentype=None): 
    """ Identifying a name in If_Then_Stmt node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_value)

def search_Level_4_Expr(cfg, stmt, node, gentype=None): 
    """ Identifying a name in Level_4_Expr node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_value)
    get_name_or_defer(cfg, stmt, node.items[2], res_value)

def search_If_Stmt(cfg, stmt, node, gentype=None): 
    """ Identifying a name in If_Stmt node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_value)
    get_name_or_defer(cfg, stmt, node.items[1], res_value)

def search_Else_If_Stmt(cfg, stmt, node, gentype=None): 
    """ Identifying a name in Else_If_Stmt node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_value)
    get_name_or_defer(cfg, stmt, node.items[1], res_value)

def search_Else_Stmt(cfg, stmt, node, gentype=None): 
    """ Identifying a name in Else_Stmt node"""
    pass

def search_Level_2_Unary_Expr(cfg, stmt, node, gentype=None): 
    """ Identifying a name in Level_2_Unary_Expr node"""
    get_name_or_defer(cfg, stmt, node.items[1], res_value)


def search_Label_Do_Stmt(cfg, stmt, node, gentype=None): 
    """ Identifying a name in Label_Do_Stmt node"""
    defer(cfg, stmt, node.items[2])

def search_Array_Constructor(cfg, stmt, node, gentype=None): 
    """ Identifying a name in Array_Constructor node"""
    get_name_or_defer(cfg, stmt, node.items[1], res_value) 

def search_Array_Section(cfg, stmt, node, gentype=None): 
    """ Identifying a name in Array_Section node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_value)
    defer(cfg, stmt, node.items[1])

def search_Substring_Range(cfg, stmt, node, gentype=None): 
    """ Identifying a name in Substring_Range node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_value)
    get_name_or_defer(cfg, stmt, node.items[1], res_value)

def search_Select_Case_Stmt(cfg, stmt, node, gentype=None): 
    """ Identifying a name in Select_Case_Stmt node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_value)

def search_Case_Stmt(cfg, stmt, node, gentype=None): 
    """ Identifying a name in Case_Stmt node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_value)

def search_Case_Selector(cfg, stmt, node, gentype=None): 
    """ Identifying a name in Case_Selector node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_value)

def search_Call_Stmt(cfg, stmt, node, gentype=None): 
    """ Identifying a name in Call_Stmt node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_subroutine)
...
def search_Char_Literal_Constant(cfg, stmt, node, gentype=None): 
    """ Identifying a name in Char_Literal_Constant node"""
    if node.items[1]:
        get_name_or_defer(cfg, stmt, Fortran2003.Name(node.items[1]), res_typedecl)
    #get_name_or_defer(cfg, stmt, node.items[0], res_typedecl)

def search_Length_Selector(cfg, stmt, node, gentype=None): 
    """ Identifying a name in Length_Selector node"""
    for item in node.items:
        get_name_or_defer(cfg, stmt, item, res_value)

def search_Type_Param_Value(cfg, stmt, node, gentype=None): 
    """ Identifying a name in Type_Param_Value node"""
    # NOTE: need to verify its content structure
    if node.item:
        get_name_or_defer(cfg, stmt, node.item, res_value)

def search_Write_Stmt(cfg, stmt, node, gentype=None): 
    """ Identifying a name in Write_Stmt node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_value)
    get_name_or_defer(cfg, stmt, node.items[1], res_value)

def search_Read_Stmt(cfg, stmt, node, gentype=None): 
    """ Identifying a name in Read_Stmt node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_value)
    get_name_or_defer(cfg, stmt, node.items[1], res_value)
    get_name_or_defer(cfg, stmt, node.items[2], res_value)

def search_Io_Control_Spec(cfg, stmt, node, gentype=None): 
    """ Identifying a name in Io_Control_Spec node"""
    get_name_or_defer(cfg, stmt, node.items[1], res_value)

def search_Stop_Stmt(cfg, stmt, node, gentype=None): 
    """ Identifying a name in Stop_Stmt node"""
    pass

def search_Contains_Stmt(cfg, stmt, node, gentype=None): 
    """ Identifying a name in Contains_Stmt node"""
    pass

def search_Subscript_Triplet(cfg, stmt, node, gentype=None): 
    """ Identifying a name in Subscript_Triplet node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_value) 
    get_name_or_defer(cfg, stmt, node.items[1], res_value) 
    get_name_or_defer(cfg, stmt, node.items[2], res_value) 

def search_Interface_Stmt(cfg, stmt, node, gentype=None):
    """ Identifying a name in Interface_Stmt node"""
    pass

def search_Procedure_Stmt(cfg, stmt, node, gentype=None):
    """ Identifying a name in Procedure_Stmt node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_subprogram)

def search_Prefix(cfg, stmt, node, gentype=None):
    """ Identifying a name in Prefix node"""
    for item in node.items:
        get_name_or_defer(cfg, stmt, node.items[0], res_anything)

def search_Prefix_Spec(cfg, stmt, node, gentype=None):
...
def search_Logical_Literal_Constant(cfg, stmt, node, gentype=None):
    """ Identifying a name in Logical_Literal_Constant node"""
    if node.items[1]:
        get_name_or_defer(cfg, stmt, Fortran2003.Name(node.items[1]), res_typedecl)
    #get_name_or_defer(cfg, stmt, node.items[1], res_typedecl)

def search_Access_Spec(cfg, stmt, node, gentype=None):
    """ Identifying a name in Access_Spec node"""
    pass

def search_And_Operand(cfg, stmt, node, gentype=None):
    """ Identifying a name in And_Operand node"""
    get_name_or_defer(cfg, stmt, node.items[1], res_value)

def search_Equiv_Operand(cfg, stmt, node, gentype=None):
    """ Identifying a name in Equiv_Operand node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_value)
    get_name_or_defer(cfg, stmt, node.items[2], res_value)

def search_Or_Operand(cfg, stmt, node, gentype=None):
    """ Identifying a name in Or_Operand node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_value)
    get_name_or_defer(cfg, stmt, node.items[2], res_value)

def search_Where_Construct_Stmt(cfg, stmt, node, gentype=None):
    """ Identifying a name in Where_Construct_Stmt node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_value)

def search_Elsewhere_Stmt(cfg, stmt, node, gentype=None):
    """ Identifying a name in Elsewhere_Stmt node"""
    get_name_or_defer(cfg, stmt, node.items[1], res_value)

def search_Suffix(cfg, stmt, node, gentype=None):
    """ Identifying a name in Suffix node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_value)
    get_name_or_defer(cfg, stmt, node.items[1], res_value)

def search_Declaration_Type_Spec(cfg, stmt, node, gentype=None):
    """ Identifying a name in Declaration_Type_Spec node"""
    get_name_or_defer(cfg, stmt, node.items[1], res_derivedtype)

def search_Data_Ref(cfg, stmt, node, gentype=None):
    ...
    if isinstance(node.items[0], Name):
        get_name_or_defer(cfg, stmt, node.items[0], res_typestmt, gentype=gentype)
    elif isinstance(node.items[0], Part_Ref):
        get_name_or_defer(cfg, stmt, node.items[0].items[0], res_typestmt, gentype=gentype) 
        get_name_or_defer(cfg, stmt, node.items[0].items[1], res_value) 

    for item in node.items[1:]:
        if isinstance(item, Name): pass
        elif isinstance(item, Part_Ref):
            get_name_or_defer(cfg, stmt, item.items[1], res_value)
        elif item is None: pass
        else: raise ProgramException('Unknown type: %s'%item.__class)

def search_Structure_Constructor(cfg, stmt, node, gentype=None):
    """ Identifying a name in Structure_Constructor node"""
    #get_name_or_defer(cfg, stmt, node.items[0], res_derivedtype)
    # NOTE: parser found ordinary subprogram as Structure_Constructor
    get_name_or_defer(cfg, stmt, node.items[0], res_value + res_derivedtype)
    get_name_or_defer(cfg, stmt, node.items[1], res_value)

def search_Binary_Constant(cfg, stmt, node, gentype=None):
    """ Identifying a name in Binary_Constant node"""
    pass

def search_Octal_Constant(cfg, stmt, node, gentype=None):
    """ Identifying a name in Octal_Constant node"""
    pass

def search_Hex_Constant(cfg, stmt, node, gentype=None):
    """ Identifying a name in Hex_Constant node"""
    pass

def search_Intrinsic_Stmt(cfg, stmt, node, gentype=None):
    """ Identifying a name in Intrinsic_Stmt node"""
    pass
    #get_name_or_defer(cfg, stmt, node.items[1], res_subprogram)

def search_Derived_Type_Stmt(cfg, stmt, node, gentype=None):
    """ Identifying a name in Derived_Type_Stmt node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_value)
    get_name_or_defer(cfg, stmt, node.items[2], res_value)

def search_Access_Stmt(cfg, stmt, node, gentype=None):
    """ Identifying a name in Access_Stmt node"""
    get_name_or_defer(cfg, stmt, node.items[1], res_anything)

def search_Function_Reference(cfg, stmt, node, gentype=None):
    """ Identifying a name in Function_Reference node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_function)
    get_name_or_defer(cfg, stmt, node.items[1], res_value)

def search_Return_Stmt(cfg, stmt, node, gentype=None):
    """ Identifying a name in Return_Stmt node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_function)

def search_Print_Stmt(cfg, stmt, node, gentype=None):
    """ Identifying a name in Print_Stmt node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_value)
    get_name_or_defer(cfg, stmt, node.items[1], res_value)

def search_Format(cfg, stmt, node, gentype=None):
    """ Identifying a name in Format node"""
    if hasattr(node, 'items') and len(node.items)>0:
        get_name_or_defer(cfg, stmt, node.items[0], res_value)

def search_Implicit_Stmt(cfg, stmt, node, gentype=None):
    """ Identifying a name in Implicit_Stmt node"""
    if hasattr(node, 'items') and len(node.items)>0:
        get_name_or_defer(cfg, stmt, node.items[0], res_value)

def search_Exit_Stmt(cfg, stmt, node, gentype=None):
    """ Identifying a name in Exit_Stmt node"""
    pass

def search_Pointer_Assignment_Stmt(cfg, stmt, node, gentype=None):
    """ Identifying a name in Pointer_Assignment_Stmt node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_ptr_object) # data pointer obj or procedure pointer obj
    get_name_or_defer(cfg, stmt, node.items[2], res_target) # data target or procedure target

def search_Proc_Component_Ref(cfg, stmt, node, gentype=None):
    """ Identifying a name in Proc_Component_Ref node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_value, gentype=gentype)
    # Type definition may handle a procedure component name?
    #get_name_or_defer(cfg, stmt, node.items[2], res_value)

def search_Io_Unit(cfg, stmt, node, gentype=None):
    """ Identifying a name in Io_Unit node"""
    if hasattr(node, 'items') and len(node.items)>0:
        get_name_or_defer(cfg, stmt, node.items[0], res_value)

def search_Level_3_Expr(cfg, stmt, node, gentype=None):
    """ Identifying a name in Level_3_Expr node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_value)
    get_name_or_defer(cfg, stmt, node.items[2], res_value)

def search_Open_Stmt(cfg, stmt, node, gentype=None):
    """ Identifying a name in Open_Stmt node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_value)
    get_name_or_defer(cfg, stmt, node.items[1], res_value)

def search_Connect_Spec(cfg, stmt, node, gentype=None):
    """ Identifying a name in Connect_Spec node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_value)
    get_name_or_defer(cfg, stmt, node.items[1], res_value)

def search_Endfile_Stmt(cfg, stmt, node, gentype=None):
    """ Identifying a name in Endfile_Stmt node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_value)
    get_name_or_defer(cfg, stmt, node.items[1], res_value)

def search_Position_Spec(cfg, stmt, node, gentype=None):
    """ Identifying a name in Position_Spec node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_value)
    get_name_or_defer(cfg, stmt, node.items[1], res_value)

def search_Close_Stmt(cfg, stmt, node, gentype=None):
    """ Identifying a name in Close_Stmt node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_value)
    get_name_or_defer(cfg, stmt, node.items[1], res_value)

def search_Close_Spec(cfg, stmt, node, gentype=None):
    """ Identifying a name in Close_Spec node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_value)
    get_name_or_defer(cfg, stmt, node.items[1], res_value)

def search_Label(cfg, stmt, node, gentype=None):
    """ Identifying a name in Label node"""
    pass

def search_Io_Implied_Do(cfg, stmt, node, gentype=None):
    """ Identifying a name in Io_Implied_Do node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_value)
    get_name_or_defer(cfg, stmt, node.items[1], res_value)

def search_Io_Implied_Do_Control(cfg, stmt, node, gentype=None):
...
def search_Format_Stmt(cfg, stmt, node, gentype=None):
    """ Identifying a name in Format_Stmt node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_value)
    # No need for searching format-items?
    #get_name_or_defer(cfg, stmt, node.items[1], res_value)

def search_Format_Specification(cfg, stmt, node, gentype=None):
    """ Identifying a name in Format_Specification node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_value)
    get_name_or_defer(cfg, stmt, node.items[1], res_value)
    get_name_or_defer(cfg, stmt, node.items[2], res_value)

def search_Format_Item_C1002(cfg, stmt, node, gentype=None):
    """ Identifying a name in Format_Item_C1002 node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_value)
    get_name_or_defer(cfg, stmt, node.items[1], res_value)

def search_Control_Edit_Desc(cfg, stmt, node, gentype=None):
    """ Identifying a name in Control_Edit_Desc node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_value)
    get_name_or_defer(cfg, stmt, node.items[1], res_value)

def search_Format_Item(cfg, stmt, node, gentype=None):
    """ Identifying a name in Format_Item node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_value)
    get_name_or_defer(cfg, stmt, node.items[1], res_value)

def search_Alloc_Opt(cfg, stmt, node, gentype=None):
    """ Identifying a name in Alloc_Opt node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_value)
    get_name_or_defer(cfg, stmt, node.items[1], res_value)

def search_Deallocate_Stmt(cfg, stmt, node, gentype=None):
    """ Identifying a name in Deallocate_Stmt node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_value)
    get_name_or_defer(cfg, stmt, node.items[1], res_value)

def search_Cycle_Stmt(cfg, stmt, node, gentype=None):
    """ Identifying a name in Cycle_Stmt node"""
    pass

def search_External_Stmt(cfg, stmt, node, gentype=None):
    """ Identifying a name in External_Stmt node"""
    get_name_or_defer(cfg, stmt, node.items[1], \
        [ TypeDeclarationStatement, Function, Subroutine ])

def search_Case_Value_Range(cfg, stmt, node, gentype=None):
    """ Identifying a name in Case_Value_Range node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_value)
    get_name_or_defer(cfg, stmt, node.items[1], res_value)

def search_Forall_Construct_Stmt(cfg, stmt, node, gentype=None):
    """ Identifying a name in Forall_Construct_Stmt node"""
    get_name_or_defer(cfg, stmt, node.items[1], res_value)

def search_Forall_Header(cfg, stmt, node, gentype=None):
    """ Identifying a name in Forall_Header node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_value)
    get_name_or_defer(cfg, stmt, node.items[1], res_value)

def search_Forall_Triplet_Spec(cfg, stmt, node, gentype=None):
...
def search_Goto_Stmt(cfg, stmt, node, gentype=None):
    """ Identifying a name in Goto_Stmt node"""
    pass

def search_Continue_Stmt(cfg, stmt, node, gentype=None):
    """ Identifying a name in Continue_Stmt node"""
    pass

def search_Wait_Stmt(cfg, stmt, node, gentype=None):
    """ Identifying a name in Wait_Stmt node"""
    get_name_or_defer(cfg, stmt, node.items[1], res_value)

def search_Wait_Spec(cfg, stmt, node, gentype=None):
    """ Identifying a name in Wait_Spec node"""
    if hasattr(node, 'items') and len(node.items)>0:
        for item in node.items:
            get_name_or_defer(cfg, stmt, item, res_value)

def search_Rewind_Stmt(cfg, stmt, node, gentype=None):
    """ Identifying a name in Rewind_Stmt node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_value)
    get_name_or_defer(cfg, stmt, node.items[1], res_value)

def search_Flush_Stmt(cfg, stmt, node, gentype=None):
    """ Identifying a name in Flush_Stmt node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_value)
    get_name_or_defer(cfg, stmt, node.items[1], res_value)

def search_Import_Stmt(cfg, stmt, node, gentype=None):
    """ Identifying a name in Import_Stmt node"""
    get_name_or_defer(cfg, stmt, node.items[1], res_anything)

def search_Block_Data_Stmt(cfg, stmt, node, gentype=None):
    """ Identifying a name in Block_Data_Stmt node"""
    # NOTE: Temporary solution
    pass

def search_Data_Stmt(cfg, stmt, node, gentype=None):
    """ Identifying a name in Data_Stmt node"""
    if hasattr(node, 'items') and len(node.items)>0:
        for item in node.items:
            get_name_or_defer(cfg, stmt, item, res_typedecl)

def search_Data_Stmt_Value(cfg, stmt, node, gentype=None):
    """ Identifying a name in Data_Stmt_Value node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_typedecl)
    get_name_or_defer(cfg, stmt, node.items[1], res_typedecl)

def search_Save_Stmt(cfg, stmt, node, gentype=None):
    """ Identifying a name in Save_Stmt node"""
    get_name_or_defer(cfg, stmt, node.items[1], res_typedecl)

def search_Asynchronous_Stmt(cfg, stmt, node, gentype=None):
    """ Identifying a name in Asynchronous_Stmt node"""
    get_name_or_defer(cfg, stmt, node.items[1], res_typedecl)

def search_Allocatable_Stmt(cfg, stmt, node, gentype=None):
    """ Identifying a name in Allocatable_Stmt node"""
    get_name_or_defer(cfg, stmt, node.items[1], res_typedecl)

def search_Common_Stmt(cfg, stmt, node, gentype=None):
    """ Identifying a name in Common_Stmt node"""
    if hasattr(node, 'items') and len(node.items)>0:
        for itemlist in node.items:
            for name, _item in itemlist:
                get_name_or_defer(cfg, stmt, _item, res_value)

def search_Data_Stmt_Set(cfg, stmt, node, gentype=None):
    """ Identifying a name in Data_Stmt_Set node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_value)
    get_name_or_defer(cfg, stmt, node.items[1], res_value)

def search_Dimension_Stmt(cfg, stmt, node, gentype=None):
...
def search_Equivalence_Stmt(cfg, stmt, node, gentype=None):
    """ Identifying a name in Equivalence_Stmt node"""
    get_name_or_defer(cfg, stmt, node.items[1], res_typedecl)

def search_Equivalence_Set(cfg, stmt, node, gentype=None):
    """ Identifying a name in Equivalence_Set node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_typedecl)
    get_name_or_defer(cfg, stmt, node.items[1], res_typedecl)

def search_Intent_Stmt(cfg, stmt, node, gentype=None):
    """ Identifying a name in Intent_Stmt node"""
    #get_name_or_defer(cfg, stmt, node.items[0], res_typedecl)
    get_name_or_defer(cfg, stmt, node.items[1], res_typedecl)

def search_Intent_Spec(cfg, stmt, node, gentype=None):
    """ Identifying a name in Intent_Spec node"""
    pass

def search_Namelist_Stmt(cfg, stmt, node, gentype=None):
    """ Identifying a name in Namelist_Stmt node"""
    if hasattr(node, 'items') and len(node.items)>0:
        for nlname, nlgroup in node.items:
            get_name_or_defer(cfg, stmt, nlgroup, res_typedecl)

def search_Optional_Stmt(cfg, stmt, node, gentype=None):
    """ Identifying a name in Optional_Stmt node"""
    get_name_or_defer(cfg, stmt, node.items[1], res_typedecl)

def search_Pointer_Stmt(cfg, stmt, node, gentype=None):
    """ Identifying a name in Pointer_Stmt node"""
    get_name_or_defer(cfg, stmt, node.items[1], res_anything)

def search_Protected_Stmt(cfg, stmt, node, gentype=None):
    """ Identifying a name in Protected_Stmt node"""
    get_name_or_defer(cfg, stmt, node.items[1], res_typedecl)

def search_Target_Stmt(cfg, stmt, node, gentype=None):
    """ Identifying a name in Target_Stmt node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_anything)

def search_Target_Entity_Decl(cfg, stmt, node, gentype=None):
...
def search_Volatile_Stmt(cfg, stmt, node, gentype=None):
    """ Identifying a name in Volatile_Stmt node"""
    get_name_or_defer(cfg, stmt, node.items[1], res_anything)

def search_Value_Stmt(cfg, stmt, node, gentype=None):
    """ Identifying a name in Value_Stmt node"""
    get_name_or_defer(cfg, stmt, node.items[1], res_typedecl)

def search_Backspace_Stmt(cfg, stmt, node, gentype=None):
    """ Identifying a name in Backspace_Stmt node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_value)
    get_name_or_defer(cfg, stmt, node.items[1], res_value)

def search_Forall_Stmt(cfg, stmt, node, gentype=None):
    """ Identifying a name in Forall_Stmt node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_value)
    get_name_or_defer(cfg, stmt, node.items[1], res_value)

def search_Inquire_Stmt(cfg, stmt, node, gentype=None):
    """ Identifying a name in Inquire_Stmt node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_value)
    get_name_or_defer(cfg, stmt, node.items[1], res_value)
    get_name_or_defer(cfg, stmt, node.items[2], res_value)

def search_Inquire_Spec(cfg, stmt, node, gentype=None):
    """ Identifying a name in Inquire_Spec node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_value)
    get_name_or_defer(cfg, stmt, node.items[1], res_value)

def search_Nullify_Stmt(cfg, stmt, node, gentype=None):
    """ Identifying a name in Nullify_Stmt node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_value)
    get_name_or_defer(cfg, stmt, node.items[1], res_value)

def search_Where_Stmt(cfg, stmt, node, gentype=None):
    """ Identifying a name in Where_Stmt node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_value)
    get_name_or_defer(cfg, stmt, node.items[1], res_value)

def search_Arithmetic_If_Stmt(cfg, stmt, node, gentype=None):
...
def search_Computed_Goto_Stmt(cfg, stmt, node, gentype=None):
    """ Identifying a name in Computed_Goto_Stmt node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_value)
    get_name_or_defer(cfg, stmt, node.items[1], res_value)

def search_Actual_Arg_Spec(cfg, stmt, node, gentype=None):
    """ Identifying a name in Actual_Arg_Spec node"""
    get_name_or_defer(cfg, stmt, node.items[1], res_value)

def search_Data_Pointer_Object(cfg, stmt, node, gentype=None):
...
def search_Type_Attr_Spec(cfg, stmt, node, gentype=None):
...
def search_Level_5_Expr(cfg, stmt, node, gentype=None):
    """ Identifying a name in Level_5_Expr node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_value)
    get_name_or_defer(cfg, stmt, node.items[2], res_value)

def search_Parameter_Stmt(cfg, stmt, node, gentype=None):
    """ Identifying a name in Parameter_Stmt node"""
    get_name_or_defer(cfg, stmt, node.items[1], res_value)
   
def search_Named_Constant_Def(cfg, stmt, node, gentype=None): 
    """ Identifying a name in Named_Constant_Def node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_value)
    get_name_or_defer(cfg, stmt, node.items[1], res_value)

def search_Saved_Entity(cfg, stmt, node, gentype=None):
    """ Identifying a name in Saved_Entity node"""
    if len(node.items)==3 and node.items[0]=='/' and node.items[2]=='/':
        get_name_or_defer(cfg, stmt, node.items[1], res_common)
    else:
        for item in node.items:
            get_name_or_defer(cfg, stmt, item, res_value)
...
def search_Dealloc_Opt(cfg, stmt, node, gentype=None):
    """ Identifying a name in Dealloc_Opt node"""
    get_name_or_defer(cfg, stmt, node.items[1], res_value)

def search_Generic_Spec(cfg, stmt, node, gentype=None):
    """ Identifying a name in Generic_Spec node"""
    pass

def search_Assumed_Size_Spec(cfg, stmt, node, gentype=None):
    """ Identifying a name in Assumed_Size_Spec node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_value)
    get_name_or_defer(cfg, stmt, node.items[1], res_value)

def search_Common_Block_Object(cfg, stmt, node, gentype=None):
    """ Identifying a name in Common_Block_Object node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_value)
    get_name_or_defer(cfg, stmt, node.items[1], res_value)

def search_Ac_Implied_Do(cfg, stmt, node, gentype=None):
    """ Identifying a name in Ac_Implied_Do node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_value)
    get_name_or_defer(cfg, stmt, node.items[1], res_value)

def search_Ac_Implied_Do_Control(cfg, stmt, node, gentype=None):
...
def search_Specific_Binding(cfg, stmt, node, gentype=None):
    """ Identifying a name in Specific_Binding node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_typespec + [ Interface ])
    get_name_or_defer(cfg, stmt, node.items[1], res_value)
    if node.items[3] is None:
        get_name_or_defer(cfg, stmt, node.items[2], res_subprogram)
    else:
        get_name_or_defer(cfg, stmt, node.items[3], res_subprogram)

def search_Binding_Attr(cfg, stmt, node, gentype=None):
    """ Identifying a name in Binding_Attr node"""
    pass

def search_Masked_Elsewhere_Stmt(cfg, stmt, node, gentype=None):
    """ Identifying a name in Masked_Elsewhere_Stmt node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_value)

def search_Procedure_Designator(cfg, stmt, node, gentype=None):
    """ Identifying a name in Procedure_Designator node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_value)

def search_Associate_Stmt(cfg, stmt, node, gentype=None):
    """ Identifying a name in Associate_Stmt node"""
    get_name_or_defer(cfg, stmt, node.items[1], res_value)

def search_Association(cfg, stmt, node, gentype=None):
    """ Identifying a name in Association node"""
    get_name_or_defer(cfg, stmt, node.items[2], res_value)

def search_Generic_Binding(cfg, stmt, node, gentype=None):
    """ Identifying a name in Generic_Binding node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_value)
    get_name_or_defer(cfg, stmt, node.items[2], [ Fortran2003.Specific_Binding ])

def search_Complex_Literal_Constant(cfg, stmt, node, gentype=None):
    """ Identifying a name in Complex_Literal_Constant node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_value)
    get_name_or_defer(cfg, stmt, node.items[1], res_value)

def search_Char_Length(cfg, stmt, node, gentype=None):
    """ Identifying a name in Char_Length node"""
    get_name_or_defer(cfg, stmt, node.items[1], res_value)

def search_Data_Implied_Do(cfg, stmt, node, gentype=None):
    """ Identifying a name in Data_Implied_Do node"""
    get_name_or_defer(cfg, stmt, node.items[0], res_value)
    get_name_or_defer(cfg, stmt, node.items[1], res_value, gentype=KGGenType.STATE_OUT)
    get_name_or_defer(cfg, stmt, node.items[2], res_value)
    get_name_or_defer(cfg, stmt, node.items[3], res_value)
    get_name_or_defer(cfg, stmt, node.items[4], res_value)

def search_Ac_Spec(cfg, stmt, node, gentype=None):
    """ Identifying a name in Ac_Spec node"""
    defer(cfg, stmt, node.items[0])
    get_name_or_defer(cfg, stmt, node.items[1], res_value)

def search_Sequence_Stmt(cfg, stmt, node, gentype=None):
    """ Identifying a name in Sequence_Stmt node"""
    pass

def search_Stmt_Function_Stmt(cfg, stmt, node, gentype=None):
