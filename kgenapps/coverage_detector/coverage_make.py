...
def generate_makefile(coverage_srcfiles):

    # Makefile for state
    generate_coverage_makefile(coverage_srcfiles)

def generate_coverage_makefile(coverage_srcfiles):

    #org_files = [ filepath for filepath, (srcfile, mods_used, units_used) in coverage_srcfiles ] 
    org_files = coverage_srcfiles
    #if not State.topblock['stmt'].reader.id in org_files:
    #    org_files.append(State.topblock['path'])

    with open('%s/Makefile'%(Config.coverage['cwd']), 'wb') as f:

        write(f, '# Makefile for KGEN-generated instrumentation for coverage detection')
        write(f, '')
